...
def parse_invariant_or_guard(guard_str, clocks, consts):
    if guard_str == '':
        return None
    guard_str = guard_str.replace('&&', 'and')
    guard_str= '&'.join(['(' + s + ')' for s in guard_str.split('and')])
...
### Function to add location and transitions
#
#def addLoc(self, tmpl, LocName):
#    ts = [t for t in self.templates if t == tmpl] 
#    invariant_str = str(loc.invariant)
...
#    ts.forward[location] = []
#    location.__class__.__str__ = lambda self: (self.ts.name + '.' + self.name) # TODO hack here, it's better to form a new class
...
#
#def parse_clocks2(nta): # returns Context with all the clocks in TA
#    #f = open(fn)
#    #nta = pyuppaal.NTA.from_xml(f)
#    ret = pydbm.Context()
#    #parsing global clocks
...
#def parse2(nta, context=None):
#    nta2 = copy.deepcopy(nta)
#    #currently we're looking them only in declaration section but probably we should also look in system section
#    channels = {}
#    nta.clocks = {}
#    if context:
#        nta_clock_names     = set([clock.name for clock in parse_clocks2(nta).clocks])
#        context_clock_names = set([clock.name for clock in context         .clocks])
#        assert(nta_clock_names <= context_clock_names)
#        nta.context = context
#    else:
#        nta.context = parse_clocks2(nta)
#    nta.global_consts = {}    
#    for declaration in parse_declarations(nta.declaration): # parsing global chans and clocks and const in declarations
#        if declaration.type  == 'chan':
#            channels[declaration.name] = Channel(declaration.name)
...
class NTAState:
    def __init__(self, nta):  
        ...
        for template in nta.templates:
            self.locations[template.name] = template.initlocation
    def copy(self):
        ret = copy.copy(self)
    ...
    def delay(self):
        ...
        for template in self.nta.templates:
            ret.federation &= self.locations[template.name].invariant
        return ret
    ...
    def __str__(self):
        ret = '(' +  ', '.join([t.name + '.' + self.locations[t.name].name for t in self.nta.templates]) + ')'
